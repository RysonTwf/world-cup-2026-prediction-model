...
// Use live in-tournament ratings if available (generated by live-ratings.mjs),
// otherwise fall back to the frozen calibrated snapshot.
...
try {
  ...
  ratingsSource = `live (${live.matchesApplied} WC matches applied, data through ${live.dataThrough?.slice(0, 10)})`;
} catch {
  ({ ratings } = JSON.parse(readFileSync(new URL('./data/elo-calibrated.json', import.meta.url), 'utf8')));
